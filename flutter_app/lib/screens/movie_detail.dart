...
class _MovieDetailScreenState extends State<MovieDetailScreen> {
  ...
  Future<List<Map<String, dynamic>>> fetchReviews(int movieId) async {
    try {
      final res = await _dio.get("/reviews/movie/$movieId");
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            _buildStatItem("평점", voteAverage != null ? voteAverage.toString() : "0.0", Colors.purpleAccent),
            _buildStatItem("상영시간", runtime != null ? "${runtime}분" : "정보 없음", Colors.blueAccent),
            _buildStatItem("인기도", popularity != null ? popularity.toString() : "0", Colors.orange),
          ],
        ),
      ),
    );
